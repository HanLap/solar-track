...
class SolarMax ( object ):
  ...
  def __repr__(self):
    return f'SolarMax[{self.__host}:{self.__port} / socket={self.__socket}]' 

  def __str__(self):
    return f'SolarMax[{self.__host}:{self.__port} / socket={self.__socket} / inverters={self.inverters()}]'

  def __disconnect(self):
    try:
      DEBUG(f'Closing open connection to {self.__host}:{self.__port}')
  ...
  def __connect(self):
    self.__disconnect()
    DEBUG(f'establishing connection to {self.__host}:{self.__port}...')
    try:
      # Python 2.5
      ...
      DEBUG(f'connection to {self.__host}:{self.__port} failed')
      self.__connected = False
      self.__allinverters = False
  ...
  def __receive(self):
    try:
      ...
      tmp = ''
      while True:
         tmp = self.__socket.recv(1).decode('ascii')
         data += tmp
         if len(tmp) < 1 or tmp == '}':
           break
         tmp = ''
  ...
  def __parse(self, answer):
    ...
    if answer[0] != '{' or answer[-1] != '}':
      raise ValueError(f'malformed answer: {answer}')
    raw_answer = answer
    answer = answer[1:-1]
    checksum = answer[-4:]
    content = answer[:-4]
    ...
    for item in content.split(';'):
      ...
      if key not in query_types:
        raise NotImplementedError("Don't know {item}")
      data[key] = value
  ...
  def __build_query(self, id, values, qtype=100):
    ...
    querystring = f'FB;{self.hexval(id, 2)};{self.hexval(l)}{querystring}'
  ...
  def detect_inverters(self):
    ...
    for inverter in self.__inverter_list:
      try:
        DEBUG(f'searching for {inverter} (socket: {self.__socket})')
        ...
        if data['TYP'] in inverter_types.keys():
          ...
          DEBUG('Unknown inverter type: %s (ID S %s)'.format(data['TYP'], data['ADR']))
      except Exception as e:
        print(e, type(e))
        DEBUG(f'Inverter #{inverter} not found.... {e}')
        self.__allinverters = False
