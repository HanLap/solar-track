...
export async function up(db) {
  ...
  await db.insertInto('solar_plant').values({
    name: 'Solar Plant 1',
    ip: '192.168.178.123',
    port: 12345,
    startAddr: 9,
  ...
  await sql`insert into measurement_tmp (inverter_id, pac, pdc, kdy, kt0, fdat, created_at) select inverter_id, pac, pdc, kdy, kt0, fdat, created_at from measurement`.execute(db);

  await db.schema.dropTable('inverter').execute();
  await db.schema.alterTable('inverter_tmp').renameTo('inverter').execute();

  await sql`insert into measurement (inverter_id, pac, pdc, kdy, kt0, fdat, created_at) select inverter_id, pac, pdc, kdy, kt0, fdat, created_at from measurement_tmp`.execute(db);
